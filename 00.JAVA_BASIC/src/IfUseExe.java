...
public class IfUseExe {
	...
	public static void main(String[] args) {
		/*
		 * 윤년여부출력
		 *  평년 (1900, 2100, 2200, 2300, 2500)
		 */
		int year = 2500;
		
		if (((year%4==0) || (year%400==0)) && (year%100!=0)) {
			System.out.printf("%d 는 윤년입니당~\n", year);
		} else {
			System.out.printf("%d 는 평년이에용~\n", year);
		}
		/*
		 문자판단 
		 	1.한글여부 
		...
		if (c>='a' && c<='z') {
			System.out.println(c + " 는 알파벳소문자 입니다.");
		}
		if (c>='0' && c<='9') {
		...
		if ((idFirstLetter >='A' && idFirstLetter <= 'Z') || (idFirstLetter >='a' && idFirstLetter<='z')) {
			System.out.println(idFirstLetter+" 는아이디로 적합");
		} else {
			System.out.println(idFirstLetter+" 는아이디로 부적합");
