...
3. 이벤트 소스객체와 이벤트 핸들러(리스너)객체 연결 :
 				...
 				이벤트 소스에서 실제적으로 이벤트가 발생이 되면 처리할 수 있도록 
	  ...
	  ex> 버튼객체.addActionListener(핸들러 객체);

 */

public class ActionEventJFrame extends JFrame { //상속
	private JButton northB;   //변수 선언
	private JButton southB;

	public ActionEventJFrame() {
		this.setTitle("ActionEvent");
		this.getContentPane().setLayout(new BorderLayout());
		northB = new JButton("이벤트소스[NORTH]");
		southB = new JButton("이벤트소스[SOUTH]");
		this.getContentPane().add(northB, BorderLayout.NORTH);
		this.getContentPane().add(southB, BorderLayout.SOUTH);
		
		// event
		northB.addActionListener(new NorthButtonActionEventHandler());
	...
	public class NorthButtonActionEventHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			System.out.println("event 발생시 실행메소드" + Thread.currentThread().getName());
			System.out.println(e);
			/*
			 * 이벤트 객체로부터 이벤트 소스 객체 참조 얻기
			 */
